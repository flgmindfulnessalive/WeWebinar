-- Add an optional phone/mobile number to registrants, captured from the
-- public registration form's new (optional) phone field. Nullable, no
-- default -- the field is opt-in, existing rows simply have no phone.
alter table public.registrants add column phone text;

-- register_for_webinar() needs a matching optional p_phone parameter to
-- persist it. This is a straight copy of the function body from
-- 20260823000005_exclude_weekends_register_rpc.sql (the latest prior
-- version) with only the phone plumbing added -- no other logic changes.
