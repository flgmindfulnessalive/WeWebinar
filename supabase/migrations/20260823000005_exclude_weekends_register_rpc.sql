-- register_for_webinar() must reject a computed_session_start that falls
-- on a weekend when the matched schedule has exclude_weekends set, same
-- as the day_of_week check right above it -- otherwise a client could
-- submit a Saturday/Sunday timestamp directly and bypass the UI's
-- weekend-free occurrence list.
