...
export default async function RegisterPage({
  ...
  const [presenter, { data: schedules }, { data: plan }, { data: waitingRoom }] = await Promise.all([
  ...
  // the same playback window, including just-in-time registrants whose
  // personal window happens to overlap a fixed slot -- not only people who
  // picked that exact slot), not a lifetime total for the webinar -- the
  // same recording can be "replayed" as unlimited sessions over time.
  // get_webinar_occurrence_spots_taken() mirrors the real
  // enforce_attendee_limit() trigger's own overlap-window formula server-
  // side and is SECURITY DEFINER specifically because this page runs as an
  // anonymous visitor, who has no RLS access to read `registrants` rows
  // directly -- a prior version of this query tried to `select` from
  // `registrants` as anon, which silently returned zero rows every time
  // (registrants_select_members requires account membership), so this page
  // always showed full availability regardless of real registrations.
  const rawOccurrences = hasFixedSlots ? computeUpcomingOccurrences(schedules ?? [], { limit: 5 }) : [];
  const spotsTakenByStart = new Map<string, number>();
  if (hasFixedSlots && rawOccurrences.length > 0) {
    const { data: spotsTaken } = await supabase.rpc("get_webinar_occurrence_spots_taken", {
      p_webinar_id: webinar.id,
      p_session_starts: rawOccurrences.map((o) => o.startsAt.toISOString()),
    });
    for (const row of spotsTaken ?? []) {
      spotsTakenByStart.set(new Date(row.session_start).toISOString(), row.concurrent_count);
    }
  }

  const occurrences = rawOccurrences.map((o) => {
    const startsAt = o.startsAt.toISOString();
    const registeredCount = spotsTakenByStart.get(startsAt) ?? 0;
    return {
