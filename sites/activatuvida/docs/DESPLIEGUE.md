...
Avance (25-sep-2026): ✅ zona creada en Cloudflare (registros de Canva borrados; se conservan `_domainconnect` y `_dmarc`) · ✅ nameservers cambiados en GoDaddy a `demi.ns.cloudflare.com` / `newt.ns.cloudflare.com` · ✅ **zona activa** en Cloudflare · ✅ token corregido (`/user/tokens/verify` → *active*) · ✅ `build`, `check`, `build:production`, `check:production` y `test:worker` superados · ✅ **vista previa publicada**: <https://activatuvida-x39-preview.activatuvida-x39.workers.dev/X39> · ✅ **producción publicada** (25-sep-2026, confirmada por el propietario en el chat): <https://activatuvida.life/X39>. Wrangler creó los dominios personalizados `activatuvida.life` y `www.activatuvida.life` (registros DNS y certificado); no se tocó ningún otro registro · ✅ reproducción de Vimeo en el móvil confirmada por el propietario · ✅ video del fundador cambiado a 1222389389 y redesplegado (vista previa y producción).

> Pendiente opcional: `http://activatuvida.life/X39` responde 200 sin pasar a https. Para forzarlo, en Cloudflare → *SSL/TLS → Edge Certificates → Always Use HTTPS* (el token actual no tiene permiso para cambiar ajustes de zona).

> **Archivos incrustados en el Worker (25-sep-2026).** Con `wrangler.toml` / `wrangler.preview.toml`, la subida de Workers Static Assets falla con `401` en `/workers/assets/upload`: ese endpoint exige un JWT temporal propio y el proxy de Claude Code sustituye la cabecera `Authorization` por el token de API. Por eso, desde este entorno se despliega con `deploy/worker-inline.js`, que incluye todo `dist/` dentro del script (2,7 MB; 2,6 MB gzip, bajo el límite de 3 MB del plan Free: vigilar al añadir imágenes) y reproduce el binding `ASSETS`: tipos MIME, reglas de `_headers`, ETag/304 y 404 para lo demás. Mismo Worker y mismos dominios:
>
