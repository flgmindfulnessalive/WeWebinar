...
**Update, 2026-09-13:** three of the four `REMEDIATION_ROADMAP.md` horizons are now worked. "Immediate (24-48h)" is fully remediated (4 items). "Before scaling paid ad-traffic campaigns" is **13 of 15 fixed**. "Next 30 days" is **15 of 19 fixed** (2 more partially — missing indexes added but a retention policy is still a product decision; Vimeo thumbnails added but direct-URL is a deliberate product call). Notably, this last batch closed real security gaps beyond the original "video/analytics" focus: an SSRF in outbound webhooks (WW-P2-015) and two IDOR/re-parenting bugs (WW-P2-016, WW-P2-017) are now fixed. Every deferred item across all three horizons has a documented reason — a live-DB/live-Whop/live-DST verification this sandbox couldn't run, a pending product decision, or (for WW-RLS-H1's regression-test suite) no Supabase CLI/Docker access to stand up a real Postgres instance to test RLS against. All fixes validated clean (tsc/eslint/vitest, 152/152 tests passing as of the latest batch). See `REMEDIATION_ROADMAP.md` and `FINDINGS.md` for the full account of each. The rest of this document is preserved as originally written (Phase 2 synthesis) except where marked with a 2026-09-13 update note.
