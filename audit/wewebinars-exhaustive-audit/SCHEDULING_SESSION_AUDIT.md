...
**Remediation status (2026-09-13):** WW-P1-001 was retracted as a false positive (see its own note below/in `FINDINGS.md`). WW-P2-001 (mid-session edits/archiving) and WW-P3-001 (spots-left display) are both now **FIXED** — the latter turned out worse than originally scoped: the anon-role query behind it was fully blocked by RLS (always 0 rows), so the registration page always showed full availability regardless of real registration counts; fixed via a new SECURITY DEFINER RPC. WW-P3-002 (DST spring-forward gap) is now also **FIXED** (2026-09-13), reversing an earlier deferral in this same file — it turned out not to need live testing at all: Node's `Intl` carries the real IANA tz-transition database, so the bug (`zonedWallTimeToUtc` silently returning an instant an hour earlier than requested for a nonexistent spring-forward wall time) was fully reproducible and fixable with a deterministic unit test. See `src/lib/scheduling.test.ts` and `FINDINGS.md`/`REMEDIATION_ROADMAP.md` for the full resolutions.
