...
| Are refunds/chargebacks handled? | **PASS** *(2026-09-13)* | Was FAIL, then CONDITIONAL PASS (ops-alert-only, pending the chargeback-policy question in `OPEN_QUESTIONS.md` §5.4). **Now fully FIXED** — the product owner confirmed WeWebinars has no refund policy, so `dispute.created`/`refund.created` now suspend the account automatically alongside the ops alert. |
...
**Update, 2026-09-13:** all four `REMEDIATION_ROADMAP.md` horizons are now worked — 37 findings fixed (2 partially) across "Immediate," "before scaling," "Next 30 days," and "Next 90 days," plus 2 retracted as false positives (WW-P1-001, WW-P4-001), plus both remaining product-decision questions (WW-P2-003, WW-P3-003's chargeback policy) answered by the product owner and closed. The headline FAIL scores that drove the original "not ready" call — video reliability, analytics/metric integrity, IDOR/SSRF exposure, refund/chargeback handling — are now all PASS. What's left open is a short, explicitly-documented list: **WW-P2-013** (video-availability monitoring/alerting — not attempted, needs new infrastructure), **WW-P2-018** (Whop webhook idempotency on the main billing path — investigated and deliberately deferred, the obvious fix was found unsafe), **WW-P3-008/WW-P3-009** (analytics retention policy / concurrent-viewer overcount — partial or low-priority-disabled), and a full e2e/integration test framework plus the broader RLS/Whop test-coverage buildout (needs a live Postgres instance this sandbox doesn't have).
