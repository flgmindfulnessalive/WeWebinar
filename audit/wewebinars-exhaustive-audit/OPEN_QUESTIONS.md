...
1. ~~**Is "one Whop user owning two separate WeWebinars accounts" (WW-P2-003) an actual supported scenario**~~ — **ANSWERED 2026-09-13** by the product owner: not a supported/needed scenario — getting a second account just requires a different email address, so this isn't a real friction point. WW-P2-003 is closed as informational ("working as designed"); the `billing_customer_id` UNIQUE constraint stays as-is, no code change.
2. **Is the current "exports always show all-time totals" behavior (WW-P2-005) an intentional, permanent product decision**, or should exports respect the dashboard's active date filter? Either answer is a legitimate product choice — the finding is that the current behavior is silent and undocumented, not that either choice is wrong. *(Resolved during remediation in favor of exports always matching the dashboard's selected range — see `REMEDIATION_ROADMAP.md`.)*
3. **Is the wall-clock-driven "completion" signal (WW-P1-010) an acceptable tradeoff** given its documented rationale (tolerating ordinary cross-origin player lag), with only the *total-failure* edge case needing a fix — or does the business want tighter correctness even for the common case? The recommended fix (track whether playback ever started) is scoped narrowly to the edge case on the assumption the answer is "yes, keep the current design, just patch the total-failure gap" — worth confirming that's actually the intended scope before building it. *(Resolved during remediation with exactly that narrow scope — see `REMEDIATION_ROADMAP.md`.)*
4. ~~**What is WeWebinars' actual chargeback/refund policy**~~ — **ANSWERED 2026-09-13** by the product owner: there is no refund policy. WW-P3-003 is now implemented as auto-suspend on `dispute.created`/`refund.created` (in addition to the existing ops alert) — a dispute or refund is treated as a suspension-worthy violation, not a wait-for-a-human case, since nothing WeWebinars agreed to justifies the money leaving. See `src/app/api/webhooks/whop/route.ts`'s `suspendAccountForDispute`.
